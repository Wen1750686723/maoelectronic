	        ...
	        if($site_url!=mc_site_url()) {
		        ...
		        if(is_numeric($page)) {
			        if($_GET['keyword']) {
				        ...
				        if(mc_option('home_mode')=='post') {
				        	$condition['meta_key'] = 'tuisong';
				        	$condition['type'] = 'basic';
				        	$this->page = M('meta')->where($condition)->order('id desc')->page($page,mc_option('page_size'))->getField('page_id',true);
				        	$count = M('meta')->where($condition)->count();
						    $this->assign('count',$count);
						    $this->assign('page_now',$page);
					        $this->theme(mc_option('theme'))->display('Home/index-post');
				        } else {
