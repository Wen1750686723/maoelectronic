...
class PerformController extends Controller {
    ...
    public function publish(){
    	if(mc_user_id()) {
	    	if(mc_remove_html($_POST['title'],'all') && $_POST['content']) {
    	...
    	if(mc_is_admin() || mc_is_bianji()) {
	    	if($_POST['title'] && $_POST['content'] && is_numeric($_POST['price'])) {
		    	...
		    	if($result) {
		    		...
		    		if($_POST['price-old']>0) {
		    			mc_add_meta($result,'price-old',$_POST['price-old']);
		    		};
		    			...
		    			foreach($parameter as $key=>$val) {
			    			foreach($val as $vals) {
			    				if($vals['name']!='') {
			    					...
			    					if($vals['kucun']>0) {
				    					mc_add_meta($result,$vals['name'],$vals['kucun'],'kucun');
			    					} else {
				    					mc_add_meta($result,$vals['name'],0,'kucun');
			    					};
			    				}
			    			}
		    			}
    	...
    	if(mc_is_admin() || mc_is_bianji()) {
	    	if($_POST['title'] && $_POST['content']) {
		    	...
		    	if($result) {
		    		...
		    		$this->success('发布成功！',U('topic/index/single?id='.$result));
	    		} else {
		    		$this->error('发布失败！');
	    		}
	    	} else {
    	...
    	if(mc_is_admin() || mc_is_bianji() || mc_author_id($_POST['id'])==mc_user_id()) {
	    	if(mc_remove_html($_POST['title'],'all') && $_POST['content'] && is_numeric($_POST['id'])) {
	    		if(mc_get_page_field($_POST['id'],'type')=='pro') {
		    		...
		    		if($_POST['price-old']>0) {
		    			mc_update_meta($_POST['id'],'price-old' ,$_POST['price-old']);
		    		};
		    		if(is_numeric($_POST['xiaoliang'])) {
		    			...
		    			foreach($parameter as $key=>$val) {
			    			...
			    			foreach($val as $vals) {
			    				if($vals['name']!='') {
			    					...
			    					if($vals['kucun']>0) {
				    					mc_add_meta($_POST['id'],$vals['name'],$vals['kucun'],'kucun');
			    					} else {
				    					mc_add_meta($_POST['id'],$vals['name'],0,'kucun');
			    					};
			    				}
			    			}
		    			}
	    		...
	    		if(mc_get_page_field($_POST['id'],'type')=='pro') {
		        	...
		        	$this->success('编辑成功',U('topic/index/single?id='.$_POST['id']));
	        	} elseif(mc_get_page_field($_POST['id'],'type')=='zhongchou') {
		        	$this->success('编辑成功',U('zhongchou/index/single?id='.$_POST['id']));
	        	} else {
	    	...
	    	if(mc_user_id()) {
	    		if($_POST['parameter']) {
	    			...
	    			foreach($parameter as $key=>$val) {
						...
						$kucun = M('meta')->where("page_id='".mc_magic_in($id)."' AND meta_key='".mc_magic_in($val)."' AND type ='kucun'")->getField('meta_value');
						array_push($kucun_now,$kucun);
					};
					$pos = array_search(min($kucun_now), $kucun_now);
					$kucun_par = $kucun_now[$pos];
				} else {
						...
						if($cart) {
							...
							foreach($cart as $cart_id) {
								...
								if($par_old) {
									$par_count = 0;
									foreach($parameter as $key=>$val) {
						    			$par_name = M('option')->where("id='".mc_magic_in($key)."'")->getField('meta_value');
							    		$par_id = M('action')->where("page_id='".mc_magic_in($cart_id)."' AND action_key='parameter' AND user_id='".mc_user_id()."' AND action_value='$par_name|".mc_magic_in($val)."'")->getField('id');
							    		if($par_id) {
								    		$par_count++;
							    		}
						    		}
						    		...
						    		if($result) {
							    		if($_POST['parameter']) {
							    			$parameter = $_POST['parameter'];
							    			foreach($parameter as $key=>$val) {
							    				$par_name = M('option')->where("id='".mc_magic_in($key)."' AND type='pro'")->getField('meta_value');
