		        ...
		        if(mc_remove_html($_POST['title'],'all')) {
			        ...
			        if($_POST['user_avatar']) {
			        	if(mc_get_meta(mc_user_id(),'user_avatar',true,'user')) {
			        		mc_update_meta(mc_user_id(),'user_avatar',mc_magic_in(mc_save_img_base64($_POST['user_avatar'],true)),'user');
			        	} else {
				        	mc_add_meta(mc_user_id(),'user_avatar',mc_magic_in(mc_save_img_base64($_POST['user_avatar'],true)),'user');
			        	}
					};
					if($_POST['fmimg']) {
