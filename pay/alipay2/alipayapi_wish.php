...
/**************************请求参数**************************/
        ...
        //必填，不能修改
        //服务器异步通知页面路径
        $notify_url = mc_option('site_url').'/pay/alipay2_notify_url_wish.php';
        //需http://格式的完整路径，不能加?id=123这类自定义参数
        //页面跳转同步通知页面路径
        $return_url = mc_option('site_url').'/pay/alipay2_return_url_wish.php';
        //需http://格式的完整路径，不能加?id=123这类自定义参数，不能写成http://localhost/
        //卖家支付宝帐户
        $seller_email = mc_option('alipay2_seller');
        //必填
        //商户订单号
        ...
        $action['page_id'] = $_POST['id'];
		$action['user_id'] = mc_user_id();
		$action['date'] = strtotime("now");
		$action['action_key'] = 'wish_pending';
		$action['action_value'] = $_POST['price'];
		$result = M('action')->data($action)->add();
		$action['action_key'] = 'trade_wish_pending';
