		...
		<div class="row">
			...
			<?php $groups = M('page')->where('type="group"')->order('date desc')->select(); if($groups) : ?>
				...
				<?php foreach($groups as $valg) : ?>
					<li role="presentation" <?php if($group_id==$valg['id']) : ?>class="active"<?php endif; ?>>
			...
			<div id="single">
				...
				<div class="wishbox">
					...
					<div class="row">
						...
						<div class="col-sm-5 text-right">
							...
							<a href="javascript:;" rel="nofollow" class="btn btn-lg btn-primary"><i class="icon-heart"></i> 心愿已达成</a>
							<?php else : ?>
							<p class="mb-20">
								目标：<span><?php echo mc_total_wish_for($val['id']); ?></span>元 &nbsp; &nbsp; 
								已完成：<span><?php echo mc_total_wish($val['id']); ?></span>元
							</p>
							<a href="#" rel="nofollow" class="btn btn-lg btn-primary" data-toggle="modal" data-target="<?php if(mc_user_id()) : ?>#myModalWish<?php else : ?>#loginModal<?php endif; ?>"><i class="icon-key"></i> 帮助TA达成心愿</a>
							<?php endif; ?>
						</div>
					</div>
				</div>
				...
				<div class="text-center">
					...
					<a href="<?php echo U('home/perform/zhiding?id='.$val['id']); ?>" class="btn btn-primary">
